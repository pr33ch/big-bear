...
{
    response=$(curl -s http://static.bigbear.cs130.org$2)
    if [ "$response" != "$3" ]; then
...
CREATE_RES=$(curl -X POST http://static.bigbear.cs130.org/meme/create -d "image=angry_lebron.jpg&top=asdf&bottom=blahblahblah")
...
expected_response=$'<!DOCTYPE html>\n<html lang="en">\n<head>\n    <meta charset="UTF-8">\n    <title>Edit a Meme!</title>\n</head>\n<h1>Edit a Meme!</h1>\n<form action="/meme/create" method="post">\n  <select name="image">\n    <option value="angry_lebron.jpg">Angry LeBron</option>\n    <option value="bike_crash.jpg">Bike Crash</option>\n  </select><br>\n  <input type="text" name="top" placeholder="Top text..." value="asdf"><br>\n  <input type="text" name="bottom" placeholder="Bottom text..." value="blahblahblah"><br>\n  <input type="hidden" name="update" value="92983c49-434b-459b-95f0-4ae227303465">\n  <input type="submit" value="Update">\n</form>'
test_response $SERVER_PID $URI "$expected_response"
echo "Can view edit meme form"


# test meme editing -- change the image, top, and bottom text
EDIT_RES=$(curl -X POST http://static.bigbear.cs130.org/meme/create -d "image=bike_crash.jpg&top=asdf2&bottom=blahblahblah2&update=$MEME_ID")
# UPDATE_ID=$(echo $CREATE_RES| cut -d'>' -f 2)
# UPDATE_ID=$(echo $UPDATE_ID| cut -d'<' -f 1)
# match_ids $SERVER_PID $UPDATE_ID $MEME_ID # the
# echo "Same ID updated"
...
DEL_RES=$(curl http://static.bigbear.cs130.org/meme/delete?id=$MEME_ID)
URI="meme/view?id=$MEME_ID"
expected_response=$'<!DOCTYPE html>\n<html lang="en">\n<head>\n    <meta charset="UTF-8">\n    <title>404</title>\n</head>\n<body>\n404 - File Not Found\n</body>\n</html>'
test_response $SERVER_PID $URI "$expected_response"
